...
<?php
   ...
   $sql =<<<EOF
      SELECT * from BOYWIGH_QUOTES WHERE ID = $qid;
EOF;
   ...
   while($row = $ret->fetchArray(SQLITE3_ASSOC) ) {
      ...
      echo '<a class="btn btn-social-icon btn-whatsapp pull-right"  href="whatsapp://send?text='.$row['QUOTES'] .' from bwgh.ml/?q='.$row['ID'].'" data-action="share/whatsapp/share"><i class="fa fa-whatsapp"style="font-size:48px;color:green;"></i></a>';
      echo '<a class="btn btn-social-icon btn-twitter pull-right"  href="https://twitter.com/intent/tweet?text='.$row['QUOTES'] .' from&hashtags=boywigh&url=http://bwgh.ml/?q='.$row['ID'].'" ><i class="fa fa-twitter"style="font-size:48px"></i></a>';
